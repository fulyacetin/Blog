...
namespace BlogOdev.Core
{
    public class BlogRepository : IBlogRepository
    {
        ...
        {
            var authors = _session.Query<Post>()
                            .Where(c => c.Published)
                            .Select(c => c.Author).Distinct().ToList();

            var rtnVal = new List<Author>();

            authors.ForEach(c => rtnVal.Add(new Author { Name = c }));
        ...
        /// <returns></returns>
        public IList<Image> ImagesForPost(int postId)
        {
            var images = _session.Query<Image>()
        ...
        /// <param name="postId"></param>
        /// <returns></returns>
        public Image FirstImageForPost(int postId)
        {
            var image = _session.Query<Image>()
                                .Where(c => c.Post.Id == postId).FirstOrDefault();
        ...
        public void DeleteImage(int id)
        {
            using (var tran = _session.BeginTransaction())
            {
                var image = _session.Get<Image>(id);
                if (image != null) _session.Delete(image);
                tran.Commit();
            }
        }
        ...
        public IList<Post> Posts(int pageNo, int pageSize)
        {
            ...
            var rtnVal= _session.Query<Post>()
            ...
            {
                var firstImage = FirstImageForPost(item.Id);
                item.FirstImage = firstImage != null ? firstImage.ImagePath : "photo.jpg";
            }
        ...
        /// <returns></returns>
        public IList<Post> PostsForAuthor(string author, int pageNo, int pageSize)
        {
            var posts = _session.Query<Post>()
                            .Where(p => p.Published && p.Author==author)
                            .OrderByDescending(p => p.PostedOn)
                            .Skip(pageNo * pageSize)
                            .Take(pageSize)
                            .Fetch(p => p.Category)
        ...
        /// <param name="author"></param>
        /// <returns></returns>
        public int TotalPostsForAuthor(string author)
        {
            return _session.Query<Post>()
                           .Where(p => p.Published && p.Author==author)
                           .Count();
        ...
        public IList<Post> Posts(int pageNo, int pageSize, string sortColumn, bool sortByAscending)
        {
            ...
            {
                var firstImage = FirstImageForPost(post.Id);
                post.FirstImage = firstImage != null ? firstImage.ImagePath : "photo.jpg";
            }
